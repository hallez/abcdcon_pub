#' ---
#' title: ABCDCon Mixed models excluding truncated voxels
...
#' ## Setup the number of voxels to drop
num_vox <- 5
...
load(paste0(analyzed_mri_dir, halle::ensure_trailing_slash("multivariate_sanityCheck"), sprintf('group_spatial_trial_patterns_%02d_truncated_voxels%s.RData', num_vox, remove_str)))
load(paste0(analyzed_mri_dir, halle::ensure_trailing_slash("multivariate_sanityCheck"), sprintf('group_temporal_trial_patterns_%02d_truncated_voxels%s.RData', num_vox, remove_str)))
...
save(all_trials_z_better_names,file=paste0(analyzed_mri_dir, sprintf('group_%02d_truncated_voxels_z_renamed_spatial_temporal_PS_by_trial.RData', num_vox)))
...
lmm.null.CA1.left.spatial <- lme4::lmer(z_r ~ (1|subj), data = all_trials_body_DVSH_DVDH_CA1_left, REML = FALSE)
...
lmm.null.CA1.left.temporal <- lme4::lmer(z_r ~ (1|subj), data = all_trials_body_SVSH_DVSH_CA1_left, REML = FALSE)
...
lmm.null.CA23DG.left.spatial <- lme4::lmer(z_r ~ (1|subj), data = all_trials_body_DVSH_DVDH_CA23DG_left, REML = FALSE)
...
lmm.null.CA23DG.left.temporal <- lme4::lmer(z_r ~ (1|subj), data = all_trials_body_SVSH_DVSH_CA23DG_left, REML = FALSE)
...
if(SAVE_GRAPHS_FLAG == 1){
  ggplot2::ggsave(file = file.path(graph_fpath_out,
                                sprintf("sameAll_sameSome_diffAll_left_body_CA1_CA23DG_%02d_truncated_voxels%s.pdf", num_vox, remove_str)),
                  width=8, height=6)
}
...
if(SAVE_GRAPHS_FLAG == 1){
  ggplot2::ggsave(file = file.path(graph_fpath_out,
                                sprintf("sameAll_sameSome_diffAll_right_body_CA1_CA23DG_%02d_truncated_voxels%s.pdf", num_vox, remove_str)),
                  width=8, height=6)
}
