#' ---
#'  title: ABCDCon Pattern Similarity Analyses between runs &mdash; No outlier trials
...
#//TODO don't hard code (instead, read in from labels file?)
rois <- c("CA1.nii","CA2.nii","CA3.nii","DG.nii","ERC.nii","subiculum.nii",
          "35_36.nii","CA2_3_DG.nii","CA3_DG.nii", "whole_hippo.nii",
          # head/body/tail ROIs
          # created by `RSA_split_anterior_posterior.m`
          "CA1_head.nii", "CA1_body.nii", "CA1_tail.nii", "CA1_body_tail.nii",
          "CA3_head.nii", "CA3_body.nii", "CA3_tail.nii", "CA3_body_tail.nii",
          "CA2_3_DG_head.nii", "CA2_3_DG_body.nii", "CA2_3_DG_tail.nii", "CA2_3_DG_body_tail.nii",
          "CA3_DG_head.nii", "CA3_DG_body.nii", "CA3_DG_tail.nii", "CA3_DG_body_tail.nii",
          "subiculum_head.nii", "subiculum_body.nii", "subiculum_tail.nii", "subiculum_body_tail.nii",
...
for(idir in c(1:length(roi_dirs))) {
  ...
  for(iroi in c(1:length(rois))){
    ...
    for(isub in subjects) {
      ...
      cur_file <- paste(
        cur_roi_dir,'br',cur_roi,'_pattern_corr_no_outlier_trials_all_runs.mat',sep="")
...
# pattern averages
save(pattern_averages,file=paste0(analyzed_mri_dir, halle::ensure_trailing_slash("multivariate_sanityCheck"), 'group_pattern_averages_no_outlier_trials_btwn_runs.RData'))
write.csv(pattern_averages,file=paste0(analyzed_mri_dir, halle::ensure_trailing_slash("multivariate_sanityCheck"), 'group_pattern_averages_no_outlier_trials_btwn_runs.csv'))

# r values for individual trials and individual subjects
if(MIXED_MODELS_FLAG==1){
  save(spatial_trials,file=paste0(analyzed_mri_dir, halle::ensure_trailing_slash("multivariate_sanityCheck"), 'group_spatial_trial_patterns.RData'))
  write.csv(spatial_trials,file=paste0(analyzed_mri_dir, halle::ensure_trailing_slash("multivariate_sanityCheck"), 'group_spatial_trial_patterns.csv'))
  save(temporal_trials,file=paste0(analyzed_mri_dir, halle::ensure_trailing_slash("multivariate_sanityCheck"), 'group_temporal_trial_patterns.RData'))
  write.csv(temporal_trials,file=paste0(analyzed_mri_dir, halle::ensure_trailing_slash("multivariate_sanityCheck"), 'group_temporal_trial_patterns.csv'))
}
